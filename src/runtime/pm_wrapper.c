...
void *construct_pm_ptr(pm_region_reference_id reference_id,
                       pm_region_offset offset) {
  PmThinPointer thin_ptr = {.offset = offset, .reference_id = reference_id};
  return thin_ptr.ptr;
}

PmThinPointer destruct_pm_ptr(void *ptr) {
  PmThinPointer thin_ptr = {.ptr = ptr};
  return thin_ptr;
}

pm_region_reference_id pm_init_reg(PmRegionConfig region_config) {
  PmBackendContext *context =
      (PmBackendContext *)calloc(sizeof(PmBackendContext), 1);
  ...
  pm_region_reference_id ref_id = created_new
...
int pm_init(PmWrapperConfig pm_config) {
  ...
  pm_region_offset program_root_offset = config.backend->alloc(
      main_context,
      sizeof(config.main_region_config.root_size - sizeof(PmMainRegionRoot)));

  config.backend->write_object(main_context, (void *)&root->program_root,
...
void *pm_get_root() {
  PmMainRegionRoot *root =
      (PmMainRegionRoot *)pm_read_object(pm_get_root_reg(1));

...
struct hashmap {
  ...
  struct buckets *buckets_tmp;
};

/*
 * create_entry -- entry initializer
 */
static int create_entry(PMEMobjpool *pop, void *ptr, void *arg) {
...
static void create_hashmap(PMEMobjpool *pop,
                           TOID(struct hashmap_atomic) hashmap, uint32_t seed) {
  ...
  size_t sz = sizeof(struct buckets) + len * sizeof(struct entries_head);
 ...
 */
static uint64_t hash(const struct hashmap *hashmap, uint64_t value) {
  uint32_t a = hashmap->hash_fun_a;
  uint32_t b = hashmap->hash_fun_b;
  uint64_t p = hashmap->hash_fun_p;
  size_t len = hashmap->buckets->nbuckets;
 ...
 */
static void hm_atomic_rebuild_finish(PMEMobjpool *pop,
  ...
  for (size_t i = 0; i < D_RO(cur)->nbuckets; ++i) {
    while (!POBJ_LIST_EMPTY(&D_RO(cur)->bucket[i])) {
      TOID(struct entry) en = POBJ_LIST_FIRST(&D_RO(cur)->bucket[i]);
      uint64_t h = hash(&hashmap, &tmp, D_RO(en)->key);

      if (POBJ_LIST_MOVE_ELEMENT_HEAD(pop, &D_RW(cur)->bucket[i],
                                      &D_RW(tmp)->bucket[h], en, list, list)) {
 ...
 */
static void hm_atomic_rebuild(PMEMobjpool *pop,
                              TOID(struct hashmap_atomic) hashmap,
                              size_t new_len) {
  if (new_len == 0)
    new_len = D_RO(D_RO(hashmap)->buckets)->nbuckets;

  size_t sz = sizeof(struct buckets) + new_len * sizeof(struct entries_head);
 ...
 */
int hm_atomic_insert(struct hashmap *hashmap, uint64_t key, void *value) {
  struct buckets *buckets = hashmap->buckets;
  TOID(struct entry) var;

  uint64_t h = hash(hashmap, key);
  int num = 0;

  struct entry *list_head = buckets->bucket[h];
  struct entry *list_entry = list_head;
  while (list_entry) {
    if (list_entry->key == key) {
      return 1;
    }
  ...
  PMEMoid oid =
      POBJ_LIST_INSERT_NEW_HEAD(pop, &D_RW(buckets)->bucket[h], list,
                                sizeof(struct entry), create_entry, &args);
  if (OID_IS_NULL(oid)) {
 ...
 */
void *hm_atomic_remove(struct hashmap hashmap, uint64_t key) {
 ...
 */
int hm_atomic_foreach(struct hashmap_atomic hashmap,
 ...
 */
static void hm_atomic_debug(PMEMobjpool *pop,
                            TOID(struct hashmap_atomic) hashmap, FILE *out) {
  TOID(struct buckets) buckets = D_RO(hashmap)->buckets;
  TOID(struct entry) var;
...
int hm_atomic_init(PMEMobjpool *pop, TOID(struct hashmap_atomic) hashmap) {
  ...
  if (!TOID_IS_NULL(D_RO(hashmap)->buckets_tmp)) {
    ...
    if (TOID_EQUALS(D_RO(hashmap)->buckets, D_RO(hashmap)->buckets_tmp)) {

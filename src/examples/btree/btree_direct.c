...
static void btree_node_clear_value(TOID(struct btree_node) node)
{
    const char s[] = "";
    TX_MEMCPY(D_RW(node)->value, s, sizeof(s));
}

static void btree_clear_values(PMEMobjpool *pop)
{
    TOID(struct btree)
    btree = POBJ_ROOT(pop, struct btree);
    ...
    TX_BEGIN(pop)
    {
        btree_foreach(pop, D_RW(btree)->root, btree_node_clear_value);
    }
    TX_END;
}
